...
static int i2chal_read(sh2_Hal_t *self, uint8_t *pBuffer, unsigned len, uint32_t *t_us) {
	// Assuming you have a defined maximum buffer size for your I2C communication
	const size_t i2c_buffer_max = /* define your max buffer size here */;
	int32_t error;

	// Read header
	I2C_Data header_data = {
		.address = 0x4A,
		.msgOut = NULL,
		.lenOut = 0,
		.msgIn = header,
	...
	error = I2cReadDataWait(&header_data, /* define your delay here */, /* define your max block time here */);
	if (error != ERROR_NONE) {
		return 0; // or appropriate error handling
	}

	// Determine amount to read from the header
	uint16_t packet_size = (uint16_t)header[0] | (uint16_t)header[1] << 8;
	packet_size &= ~0x8000; // Unset the "continue" bit

	if (packet_size > len || packet_size > i2c_buffer_max) {
		return 0; // Buffer size is insufficient
	}

	// Read the actual data
	I2C_Data data = {
		.address = /* define the I2C address here */,
		.msgOut = NULL,
		.lenOut = 0,
		.msgIn = pBuffer,
		.lenIn = packet_size
	...
	error = I2cReadDataWait(&data, /* define your delay here */, /* define your max block time here */);
	if (error != ERROR_NONE) {
		return 0; // or appropriate error handling
	}

	return packet_size; // Return the actual number of bytes read
}
...
static int i2chal_write(sh2_Hal_t *self, uint8_t *pBuffer, unsigned len) {
    ...
    */
    ...
    // Prepare the data for your I2cWriteDataWait function
    I2C_Data i2c_data = {
        .address    = 0x4A
        .msgOut     = pBuffer,
        .lenOut     = write_size,
    ...
    // Use your I2cWriteDataWait function
    int32_t result = I2cWriteDataWait(&i2c_data, /* define your max block time here */);

    // Handle the result of the I2cWriteDataWait function
