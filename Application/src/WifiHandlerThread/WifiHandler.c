...
#include "OLed/oled.h"
#include "motorDriver/motor_driver.h"
...
QueueHandle_t xQueuePublish = NULL;         ///< Queue to hold data to publish to MQTT
...
char fajr[10], dhuhr[10], asr[10], maghrib[10], isha[10];
char currTime[10];
...
static void MQTT_HandleRequests(void);
/******************************************************************************
 * Callback Functions
 ******************************************************************************/
...
/**
 * @brief Sends topic request to retrieve data for it
 * 
 * @param topic Name of topic
 */
void sendTopicRequest(const char* topic, const char* message) {
    PublishDataPacket packet;
    strncpy(packet.topic, topic, sizeof(packet.topic));
    strncpy(packet.message, message, sizeof(packet.message));
    xQueueSend(xQueuePublish, &packet, (TickType_t)10);

    LogMessage(LOG_DEBUG_LVL, "\r\nTopic request queued for topic: %s\r\n", topic);
}

/**
 * @brief Handles location topic subscription
 * 
 * @param msgData 
 */
void SubscribeHandlerLocationTopic(MessageData *msgData) {
    LogMessage(LOG_DEBUG_LVL, "\r\nLocation topic received!\r\n");
    char latitude[20], longitude[20], city[50];
    sscanf(msgData->message->payload, "%[^,],%[^,],%[^,]", latitude, longitude, city);
    LogMessage(LOG_DEBUG_LVL, "\r\nLatitude: %s\r\nLongitude: %s\r\nCity: %s\r\n", latitude, longitude, city);
	i2c_clear(); // Clear the OLED display
	i2c_setCursor(0, 0); // Set the cursor to the top-left position
	i2c_print("Latitude:"); // Print the message
	i2c_setCursor(55, 0); // Set the cursor to the top-left position
	i2c_print(latitude); // Print the message
	i2c_setCursor(0, 1); // Set the cursor to the top-left position
	i2c_print("Longitude:"); // Print the message
	i2c_setCursor(56, 1); // Set the cursor to the top-left position
	i2c_print(longitude); // Print the message
	i2c_setCursor(0, 4); // Set the cursor to the top-left position
	i2c_print("Location:"); // Print the message
	i2c_setCursor(50, 4); // Set the cursor to the top-left position
	i2c_print(city); // Print the message
	vTaskDelay(pdMS_TO_TICKS(4000));
    // TODO: use the coords
}

/**
 * @brief Handles time topic subcription
 * 
 * @param msgData 
 */
void SubscribeHandlerTimeMcu(MessageData *msgData) {
    LogMessage(LOG_DEBUG_LVL, "\r\nTime topic received!\r\n");
    char timeString[20];
    strncpy(timeString, msgData->message->payload, sizeof(timeString));
    timeString[sizeof(timeString) - 1] = '\0'; // Ensuring null-termination
    memcpy(currTime, timeString + 11, 8);
	memmove(currTime, currTime + (strspn(currTime, " ")), strlen(currTime + (strspn(currTime, " "))) + 1);
	if (timeString[18] != 'M') { timeString[18] = '\0'; }
    LogMessage(LOG_DEBUG_LVL, "\r\nDate and Time: %s\r\n", timeString);
	i2c_clear(); // Clear the OLED display
	i2c_setCursor(0, 0); // Set the cursor to the top-left position
	i2c_print("Date and Time now:"); // Print the message
	i2c_setCursor(0, 2); // Set the cursor to the top-left position
	i2c_print(timeString); // Print the message
	checkTimeToPray();
	vTaskDelay(pdMS_TO_TICKS(3000));
    // TODO
}

void checkTimeToPray(void) {
    currTime[8] = '\0';
	if (currTime[7] != 'M') { currTime[7] = '\0'; }
    fajr[8] = '\0';
    dhuhr[8] = '\0';
    asr[8] = '\0';
    maghrib[8] = '\0';
    ...
    LogMessage(LOG_DEBUG_LVL, 
    "\r\nTime to compare: %s\r\nFajr: %s\r\nDhuhr: %s\r\nAsr: %s\r\nMaghrib: %s\r\nIsha: %s PM\r\n", 
    currTime, fajr, dhuhr, asr, maghrib, isha);
    
    if (strcmp(currTime, fajr) == 0) {
        LogMessage(LOG_DEBUG_LVL, "\r\nTime to pray Fajr!\r\n");
		i2c_clear(); // Clear the OLED display
		i2c_setCursor(0, 0); // Set the cursor to the top-left position
		i2c_print("Time to pray:"); // Print the message
		i2c_setCursor(0, 2); // Set the cursor to the top-left position
		i2c_print("Fajr"); // Print the message
		for (int i = 0; i < 3; i++) { notify_user(2); }
		checkForPrayer();
		vTaskDelay(pdMS_TO_TICKS(3000));
    } else if (strcmp(currTime, dhuhr) == 0) {
        LogMessage(LOG_DEBUG_LVL, "\r\nTime to pray Dhuhr!\r\n");
		i2c_clear(); // Clear the OLED display
		i2c_setCursor(0, 0); // Set the cursor to the top-left position
		i2c_print("Time to pray:"); // Print the message
		i2c_setCursor(0, 2); // Set the cursor to the top-left position
		i2c_print("Dhuhr"); // Print the message
		for (int i = 0; i < 3; i++) { notify_user(2); }
		checkForPrayer();
		vTaskDelay(pdMS_TO_TICKS(3000));
    } else if (strcmp(currTime, asr) == 0) {
        LogMessage(LOG_DEBUG_LVL, "\r\nTime to pray Asr!\r\n");
		i2c_clear(); // Clear the OLED display
		i2c_setCursor(0, 0); // Set the cursor to the top-left position
		i2c_print("Time to pray:"); // Print the message
		i2c_setCursor(0, 2); // Set the cursor to the top-left position
		i2c_print("Asr"); // Print the message
		for (int i = 0; i < 3; i++) { notify_user(2); }
		checkForPrayer();
		vTaskDelay(pdMS_TO_TICKS(3000));
    } else if (strcmp(currTime, maghrib) == 0) {
        LogMessage(LOG_DEBUG_LVL, "\r\nTime to pray Maghrib!\r\n");
		i2c_clear(); // Clear the OLED display
		i2c_setCursor(0, 0); // Set the cursor to the top-left position
		i2c_print("Time to pray:"); // Print the message
		i2c_setCursor(0, 2); // Set the cursor to the top-left position
		i2c_print("Maghrib"); // Print the message
		for (int i = 0; i < 3; i++) { notify_user(2); }
		checkForPrayer();
		vTaskDelay(pdMS_TO_TICKS(3000));
    } else if (strcmp(currTime, isha) == 0) {
        LogMessage(LOG_DEBUG_LVL, "\r\nTime to pray Isha!\r\n");
		i2c_clear(); // Clear the OLED display
		i2c_setCursor(0, 0); // Set the cursor to the top-left position
		i2c_print("Time to pray:"); // Print the message
		i2c_setCursor(0, 2); // Set the cursor to the top-left position
		i2c_print("Isha"); // Print the message
		for (int i = 0; i < 3; i++) { notify_user(2); }
		checkForPrayer();
		vTaskDelay(pdMS_TO_TICKS(3000));
    }
...
void checkForPrayer(void) {
	LogMessage(LOG_DEBUG_LVL, "\r\nChecking for prayer!\r\n");
	hasPrayed = false;
	while (hasPrayed == false) {
		LogMessage(LOG_DEBUG_LVL, "\r\nHasn't prayed!\r\n");
		int oldZ = zAccel;
		getImuData();
		if (oldZ - zAccel > 100 | oldZ - zAccel < -100) {
			motor_stop();
			LogMessage(LOG_DEBUG_LVL, "\r\nHas prayed!\r\n");
			hasPrayed = true;
		} else {
...
void SubscribeHandlerWeatherMcu(MessageData *msgData) {
    LogMessage(LOG_DEBUG_LVL, "\r\nWeather topic received!\r\n");
    char temperatureString[6];
    strncpy(temperatureString, msgData->message->payload, sizeof(temperatureString));
    temperatureString[sizeof(temperatureString) - 1] = '\0'; // Ensuring null-termination
    LogMessage(LOG_DEBUG_LVL, "\r\nTemperature: %s\r\n", temperatureString);
	
	  i2c_clear(); // Clear the OLED display
	  i2c_setCursor(0, 0); // Set the cursor to the top-left position
	  i2c_print("Current temperature:"); // Print the message
	  i2c_setCursor(0, 2); // Set the cursor to the top-left position
	  i2c_print(temperatureString); // Print the message
	  vTaskDelay(pdMS_TO_TICKS(3000));

    // TODO
}

void SubscribeHandlerPrayerTimesMcu(MessageData *msgData) {
    LogMessage(LOG_DEBUG_LVL, "\r\nPrayer times topic received!\r\n");
    // char fajr[10], dhuhr[10], asr[10], maghrib[10], isha[20];
    sscanf(msgData->message->payload, "%[^,],%[^,],%[^,],%[^,],%s PM", fajr, dhuhr, asr, maghrib, isha);
    LogMessage(LOG_DEBUG_LVL, "\r\nFajr: %s\r\nDhuhr: %s\r\nAsr: %s\r\nMaghrib: %s\r\nIsha: %s PM\r\n", 
    fajr, dhuhr, asr, maghrib, isha);
	
	memmove(fajr, fajr + (strspn(fajr, " ")), strlen(fajr + (strspn(fajr, " "))) + 1);
	if (fajr[strlen(fajr) - 2] == ' ') {
		fajr[strlen(fajr) - 2] = '\0';
	}
	memmove(dhuhr, dhuhr + (strspn(dhuhr, " ")), strlen(dhuhr + (strspn(dhuhr, " "))) + 1);
	if (dhuhr[strlen(dhuhr) - 2] == ' ') {
		dhuhr[strlen(dhuhr) - 2] = '\0';
	}
	memmove(asr, asr + (strspn(asr, " ")), strlen(asr + (strspn(asr, " "))) + 1);
	if (asr[strlen(asr) - 2] == ' ') {
		asr[strlen(asr) - 2] = '\0';
	}
	memmove(maghrib, maghrib + (strspn(maghrib, " ")), strlen(maghrib + (strspn(maghrib, " "))) + 1);
	if (maghrib[strlen(maghrib) - 2] == ' ') {
		maghrib[strlen(maghrib) - 2] = '\0';
	}
	memmove(isha, isha + (strspn(isha, " ")), strlen(isha + (strspn(isha, " "))) + 1);
	if (isha[strlen(isha) - 2] == ' ') {
		isha[strlen(isha) - 2] = '\0';
	}
	
	i2c_clear(); // Clear the OLED display
	i2c_setCursor(0, 0); // Set the cursor to the top-left position
	i2c_print("Fajr:"); // Print the message
	i2c_setCursor(40, 0); // Set the cursor to the top-left position
	i2c_print(fajr); // Print the message
	i2c_setCursor(0, 1); // Set the cursor to the top-left position
	i2c_print("Dhuhr:"); // Print the message
	i2c_setCursor(40, 1); // Set the cursor to the top-left position
	i2c_print(dhuhr); // Print the message
	i2c_setCursor(0, 2); // Set the cursor to the top-left position
	i2c_print("Asr:"); // Print the message
	i2c_setCursor(40, 2); // Set the cursor to the top-left position
	i2c_print(asr); // Print the message
	i2c_setCursor(0, 3); // Set the cursor to the top-left position
	i2c_print("Maghrib:"); // Print the message
	i2c_setCursor(55, 3); // Set the cursor to the top-left position
	i2c_print(maghrib); // Print the message
	i2c_setCursor(0, 4); // Set the cursor to the top-left position
	i2c_print("Isha:"); // Print the message
	i2c_setCursor(40, 4); // Set the cursor to the top-left position
	i2c_print(isha); // Print the message
...
void SubscribeHandlerMotor(MessageData *msgData) {
    LogMessage(LOG_DEBUG_LVL, "\r\nMotor topic received!\r\n");
    bool motorStatus = (strncmp(msgData->message->payload, "true", 4) == 0);
    if (motorStatus) {
        LogMessage(LOG_DEBUG_LVL, "\r\nMotor status: ON\r\n");
		i2c_clear(); // Clear the OLED display
		i2c_setCursor(0, 0); // Set the cursor to the top-left position
		i2c_print("Motor status: ON"); // Print the message
		/*motor_vibrate_continuous();*/
		checkForPrayer();
    } else {
        LogMessage(LOG_DEBUG_LVL, "\r\nMotor status: OFF\r\n");
		i2c_clear(); // Clear the OLED display
		i2c_setCursor(0, 0); // Set the cursor to the top-left position
		i2c_print("Motor status: OFF"); // Print the message
		motor_stop();
    }
    // TODO
}
...
void getImuData() {
	...
	if (reg) {
		...
		zAccel = (int)acceleration_mg[2];
		
		char acceleration[60];

		LogMessage(LOG_DEBUG_LVL, "\r\nAcceleration [mg]:X %d\tY %d\tZ %d\r\n", (int)acceleration_mg[0], 
		(int)acceleration_mg[1], (int)acceleration_mg[2]);
		sprintf(acceleration, "X: %d, Y: %d, Z: %d", (int)acceleration_mg[0], (int)acceleration_mg[1], (int)acceleration_mg[2]);
		sendTopicRequest("resP", acceleration);
		/*mqtt_publish(&mqtt_inst, "/watch/imu/res", acceleration, strlen(acceleration), 2, 0);*/
		} else {
		LogMessage(LOG_DEBUG_LVL, "\r\nNo data ready! \r\n");
		mqtt_publish(&mqtt_inst, "/watch/imu/res", "\r\nNo data ready! \r\n", strlen("\r\nNo data ready! \r\n"), 2, 0);
	}
}

void SubscribeHandlerImuReq(MessageData *msgData) {
    LogMessage(LOG_DEBUG_LVL, "\r\nIMU topic received!\r\n");
    bool imuRequest = (strncmp(msgData->message->payload, "true", 4) == 0);
    LogMessage(LOG_DEBUG_LVL, "\r\nIMU request: %s\r\n", imuRequest ? "true" : "false");
    // TODO
	getImuData();
}
    ...
    switch (type) {
        ...
        case MQTT_CALLBACK_CONNECTED:
            if (data->connected.result == MQTT_CONN_RESULT_ACCEPT) {
				...
				mqtt_subscribe(module_inst, "sensor", 2, SubscribeHandlerImuReq);
                mqtt_subscribe(module_inst, LOCATION_TOPIC_RES, 2, SubscribeHandlerLocationTopic);
                mqtt_subscribe(module_inst, TIME_TOPIC_RES, 2, SubscribeHandlerTimeMcu);
                mqtt_subscribe(module_inst, MOTOR_TOPIC, 2, SubscribeHandlerMotor);
				mqtt_subscribe(module_inst, PRAYER_TOPIC_RES, 2, SubscribeHandlerPrayerTimesMcu);
...
static void MQTT_HandleRequests(void) {
	PublishDataPacket packet;
	if (xQueueReceive(xQueuePublish, &packet, 0) == pdPASS) {
		int resP = mqtt_publish(&mqtt_inst, packet.topic, packet.message, strlen(packet.message), 2, 0);
		if (hasPrayed) {
			mqtt_publish(&mqtt_inst, "hasPrayed", "Yes", 3, 2, 0);
			} else {
			mqtt_publish(&mqtt_inst, "hasPrayed", "No", 3, 2, 0);
		}
	}
}
...
static void MQTT_HandleImuMessages(void)
{
    ...
    if (pdPASS == xQueueReceive(xQueueImuBuffer, &imuDataVar, 0)) {
        ...
        mqtt_publish(&mqtt_inst, IMU_TOPIC_RES, mqtt_msg, strlen(mqtt_msg), 2, 0);
    }
}
...
void vWifiTask(void *pvParameters)
{
    ...
    xQueuePublish = xQueueCreate(10, sizeof(PublishDataPacket));

    if (xQueueWifiState == NULL || xQueueImuBuffer == NULL || xQueuePublish == NULL) {
