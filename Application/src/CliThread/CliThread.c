...
BaseType_t CLI_NotifyUser(int8_t *pcWriteBuffer, size_t xWriteBufferLen, const int8_t *pcCommandString);
/******************************************************************************
 * Defines
 ******************************************************************************/
...
static const CLI_Command_Definition_t xNotifyMotorCommand = {
	"notify_motor", // The command string to type in the CLI
	"notify_motor <type>: Triggers motor notification of a specified type (1-10).\r\n", // Help string
	(const pdCOMMAND_LINE_CALLBACK)CLI_NotifyUser, // Command handler function
...
void vCommandConsoleTask(void *pvParameters)
{
    ...
    FreeRTOS_CLIRegisterCommand(&xNotifyMotorCommand);
...
BaseType_t CLI_NeotrellProcessButtonBuffer(int8_t *pcWriteBuffer, size_t xWriteBufferLen, const int8_t *pcCommandString)
{
      ...
      port_pin_set_output_level(PIN_PA21, false); // Set GPIO Low at start
    ...
    port_pin_set_output_level(PIN_PA21, true); // Set GPIO High at end
    return pdFALSE;
}
...
BaseType_t CLI_NotifyUser(int8_t *pcWriteBuffer, size_t xWriteBufferLen, const int8_t *pcCommandString) {
	...
	// Fetch the notification type from the command string
	pcParameter = FreeRTOS_CLIGetParameter(pcCommandString, 1, &xParameterStringLength);

	// Convert the parameter to an integer
	notificationType = atoi((char *)pcParameter);

	// Validate the notification type
	if (notificationType < 1 || notificationType > 10) {
		snprintf((char *)pcWriteBuffer, xWriteBufferLen, "Invalid notification type. Please enter a value between 1 and 10.\r\n");
		return pdFALSE;
	}

	// Call the notify_user function
	notify_user(notificationType);

	snprintf((char *)pcWriteBuffer, xWriteBufferLen, "Motor notification type %d executed.\r\n", notificationType);
	return pdFALSE;
}
