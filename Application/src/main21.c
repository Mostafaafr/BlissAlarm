...
#define OLED_TASK_SIZE 128 // OLED task stack size
#define OLED_PRIORITY (tskIDLE_PRIORITY + 1) // OLED task priority
...
void vOledTask(void *pvParameters); // OLED task function
void vMotorTask(void *pvParameters); // Motor task function
/******************************************************************************
* Variables
******************************************************************************/
...
static TaskHandle_t oledTaskHandle = NULL; // OLED task handle
static TaskHandle_t motorTaskHandle = NULL; // Motor task handle
...
motor_driver_init(MOTOR_DRIVER_PIN); // Initialize the motor driver

uint8_t whoamI = 0;
(lsm6dso_device_id_get(GetImuStruct(), &whoamI));
char iAM[50];
snprintf(iAM, sizeof(iAM), "I am %x \r\n", whoamI);
SerialConsoleWriteString(iAM);
...
static void StartTasks(void)
{
    ...
    // Create OLED task
    if (xTaskCreate(vOledTask, "OLED Task", OLED_TASK_SIZE, NULL, OLED_PRIORITY, &oledTaskHandle) != pdPASS) {
        SerialConsoleWriteString("ERR: OLED task could not be initialized!\r\n");
    }
	if (xTaskCreate(vMotorTask, "Motor Task", MOTOR_TASK_STACK_SIZE, NULL, MOTOR_TASK_PRIORITY, &motorTaskHandle) != pdPASS) {
		SerialConsoleWriteString("Error: Motor task could not be created!\r\n");
	}
	if (xTaskCreate(vWifiTask, "Wifi Task", WIFI_TASK_SIZE, NULL, WIFI_PRIORITY, &wifiTaskHandle) != pdPASS) {
		SerialConsoleWriteString("Error: WiFi task could not be created!\r\n");
	}
}
...
/**************************************************************************//**
* function          vOledTask
* @brief            Task for controlling the OLED display
* @details
* @param[in]        pvParameters Parameter passed into the task
* @return           None
*****************************************************************************/
void vOledTask(void *pvParameters)
{
  ...
  i2c_print("Hello BlissAlarm!"); // Print the message
  /*vTaskDelay(pdMS_TO_TICKS(5000));*/
//   i2c_clear(); 

    for(;;) {
        // OLED task loop
        vTaskDelay(pdMS_TO_TICKS(5000)); // Delay for a period
    }
}
void vMotorTask(void *pvParameters)
{
	for (int i = 0; i < 10000; i++) {
		// Example: Vibrate the motor continuously
